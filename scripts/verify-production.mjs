...
async function run() {
  ...
  const layout = await prodPage.evaluate(() => ({
    hasMapPage: !!document.querySelector('.map-page'),
    hasTitle: !!document.querySelector('.map-page__title')?.textContent,
    hasSubtitle: !!document.querySelector('.map-page__subtitle')?.textContent,
  ...
  if (!layout.hasMapPage) failures.push('Missing .map-page wrapper');
  if (!layout.hasTitle) failures.push('Missing section title');
  if (!layout.hasCard) failures.push('Missing card container');
